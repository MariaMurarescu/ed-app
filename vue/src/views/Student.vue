<template>
  <PageComponent>
    ...
    <template v-else>
      <div v-for="lesson in studentLessons" :key="lesson.id" class="container mx-auto my-4">
        ...
        <div class="border p-4 rounded-md shadow-md">
          <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3">
            <div class="col-span-3">
              ...
              <img v-if="lesson.image" :src="lesson.image" alt="" class="w-20 h-20 object-cover rounded-full" />
            ...
            <div class="col-span-5 ml-4">
              <h1 class="text-2xl font-semibold mb-2">{{ lesson.title }}</h1>
              <p v-if="lesson.description" class="text-gray-500 mb-2">{{ lesson.description }}</p>
              <p v-if="lesson.keywords" class="text-indigo-500">{{ lesson.keywords }}</p>
              <!-- Display other lesson properties as needed -->

              <div class="flex justify-between items-center mt-3">

                <router-link :to="{ name: 'StudentLessonView', params: { slug: lesson.slug, id: lesson.id, lesson_id: lesson.id, user_id: lesson.user_id} }"
