  ...
  <PageComponent>
    <template v-slot:header>
      <div class="flex items-center justify-between">
        <!-- Create or update lesson title -->

        <h1 class="text-1xl font-bold text-gray-400">
          {{ route.params.id ? model.title : "Creează lecție" }}
        </h1>
        ...
        <div class="flex">
          <a :href="`/view/lesson/${model.slug}`" target="_blank" v-if="model.slug"
            class="flex py-2 px-4 border border-transparent text-sm rounded-md text-indigo-500 hover:bg-indigo-700 hover:text-white 
            transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mr-2">
          ...
          <button v-if="route.params.id" type="button" @click="deleteLesson()"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 -mt-1 inline-block" viewBox="0 0 20 20"
              ...
              <path fill-rule="evenodd"
                ...
                2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z"
                clip-rule="evenodd" />
            </svg>
            Șterge lecție
          </button>
