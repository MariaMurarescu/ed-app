<template>
  <div class="flex flex-col py-4 px-6 shadow-md bg-white hover:bg-gray-50 h-[470px]">
    <img :src="lesson.image_url ||
      ...
      " :alt="lesson.title" class="w-full h-48 object-cover" />
    <h4 class="mt-4 text-lg font-bold">{{ lesson.title }}</h4>
    <div v-html="lesson.description" class="overflow-hidden flex-1"></div>
    <h4 class="mt-4 text-stone-500">{{ lesson.keywords }}</h4>

    <div class="flex justify-between items-center mt-3">
      <router-link :to="{ name: 'LessonView', params: { id: lesson.id } }"
      ...
      <div class="flex items-center">
        <a :href="`/view/lesson/${lesson.slug}`" target="_blank"
        ...
        <button v-if="lesson.id" type="button" @click="emit('delete', lesson)"
